...
def cherry_pop(candidates, release, previous_release, new_branch_commits, later_branch_commits, cherries_file):
    ...
    # Write out the cherries script.
    preamble = """#!/bin/bash

set -e

# This script has been autogenerated by os-cherry-pop.py.
# Create a branch from the target branch, then execute this script to apply cherry picks.
# If a cherry pick fails to apply cleanly, resolve the merge conflicts then run `git cherry-pick --continue`.
# Alternatively, if a cherry pick should be skipped, run `git cherry-pick --skip`.
# Next, edit this script to comment out all cherry picks up to and including the one that failed to apply.
# Re-run the script, repeating the process until all cherries have been applied or skipped.

"""
    with open(cherries_file, "w") as f:
        f.write(preamble)
        for cherry in reversed(cherries):
            f.write("git cherry-pick -x %s  # %s\n" % (cherry["sha1"][:10], cherry["title"]))
    ...
    print("Wrote proposed cherry-picks to %s" % cherries_file)
...
def parse_args():
    ...
    parser.add_argument("-c", "--cherries-file", default="cherries.sh",
                        help="Path to a file to write cherry-pick commands "
                             "to. Default is 'cherries.sh'")
...
def main():
    ...
    cherry_pop(candidates, release, previous_release, new_branch_commits, later_branch_commits, cherries_file)
